...
import { registerInternalAdminRoutes } from './routes/internal-admin.js';
...
app.get('/health/db',async(_r,reply)=>{
  ...
  try{
    const r=await pool.query<{version:string}>('SELECT version FROM schema_migrations ORDER BY version DESC LIMIT 1');
    return{service:'ansarraf',database:'ok',migration:r.rows[0]?.version??null};
  }catch{return reply.code(503).send({service:'ansarraf',database:'unavailable'});}
});
app.get('/health/market-data',async(_r,reply)=>{
  if(!marketData)return reply.code(503).send({service:'ansarraf',marketData:'not-configured'});
  const health=marketData.health();
  const down=Object.values(health).filter(v=>v.status==='down').length;
  return reply.code(down===3?503:200).send({service:'ansarraf',marketData:down===0?'healthy':'degraded',providers:health});
});
app.get('/api/v1/status',async()=>({service:'ansarraf',apiVersion:'v1',status:'ready',marketData:marketData?'enabled':'disabled'}));
if(pool){
  registerMarketRoutes(app,pool,marketData);
  registerTradingRoutes(app,pool);
  ...
  app.get('/api/v1/auth/me',{preHandler:requireAuth},async(request)=>{
    ...
    const r=await pool.query('SELECT id,identity_id,external_user_id,email,status FROM customers WHERE id=$1',[id]);
    return r.rows[0]?{user:r.rows[0]}:{error:'user_not_found'};
  });
  if(marketData)await marketData.start().catch(error=>app.log.error({error},'market data initial refresh failed'));
  if(accountingOutbox)accountingOutbox.start();
