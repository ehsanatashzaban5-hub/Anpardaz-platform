...
const asText=(v:any)=>v===undefined||v===null?'0':String(v);
const normalizeStatus=(value:any):ProviderOrderStatus=>{
...
export class WallexAdapter implements LiquidityProviderAdapter{
  ...
  private async request(path:string,init:RequestInit={}){
    const response=await fetch(this.cfg.baseUrl.replace(/\/$/,'')+path,{
      ...init,
      headers:{accept:'application/json','content-type':'application/json',authorization:'Bearer '+this.cfg.apiKey,...(init.headers??{})},
  ...
  async getOrderBook(symbol:string){
    ...
    const bids=(data?.bids??[]).map((x:any)=>[asText(x[0]),asText(x[1])]);
    const asks=(data?.asks??[]).map((x:any)=>[asText(x[0]),asText(x[1])]);
    return {bids,asks,raw:body};
  }
  ...
  async getBalances():Promise<ProviderBalance[]>{
    ...
    return rows.map((x:any)=>({asset:String(x.asset??x.symbol??''),available:asText(x.available??x.free),locked:asText(x.locked??x.lockedAmount),raw:x}));
  }

  async submitOrder(request:ProviderOrderRequest):Promise<ProviderOrderResult>{
    const body=await this.request('/v1/account/orders',{method:'POST',body:JSON.stringify({
  ...
  async getOrder(clientOrderId:string,providerOrderId?:string|null):Promise<ProviderOrderResult>{
    ...
    const body=await this.request('/v1/account/order'+query);
  ...
  async cancelOrder(clientOrderId:string,providerOrderId?:string|null):Promise<ProviderOrderResult>{
    const body=await this.request('/v1/account/orders',{method:'DELETE',body:JSON.stringify({
      ...(providerOrderId?{id:providerOrderId}:{client_id:clientOrderId})
    })});
    return this.mapOrder(body,clientOrderId);
  }
  ...
  private mapOrder(body:any,clientOrderId:string):ProviderOrderResult{
    ...
    return {
      providerOrderId:x?.id!==undefined?String(x.id):x?.order_id!==undefined?String(x.order_id):null,
      clientOrderId,
      status:normalizeStatus(x?.status),
      executedQuantity:asText(x?.executedQty??x?.executed_quantity??x?.filledQuantity),
      executedQuoteAmount:asText(x?.executedQuoteAmount??x?.executed_quote_amount),
      providerFeeAmount:asText(x?.fee??x?.feeAmount??x?.fee_amount),
      providerFeeAssetSymbol:x?.feeAsset??x?.fee_asset??null,
      raw:body
    };
  }
}
