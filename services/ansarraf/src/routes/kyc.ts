...
const reviewRoles = ['admin','super_admin','operator'];
const decisionRoles = ['admin','super_admin'];
...
export function registerKycRoutes(app: FastifyInstance, pool: Pool) {
  ...
  app.get('/api/v1/admin/kyc', { preHandler: requireAuth }, async (request, reply) => {
    if (!reviewRoles.includes(ar(request).auth.role)) return reply.code(403).send({ error: 'forbidden' });
  ...
  app.post('/api/v1/admin/kyc/:id/decision', { preHandler: requireAuth }, async (request, reply) => {
    ...
    if (!decisionRoles.includes(actor.role)) return reply.code(403).send({ error: 'forbidden' });
    const id = Number((request.params as { id: string }).id);
