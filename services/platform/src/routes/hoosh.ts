...
export function registerHooshRoutes(app:FastifyInstance,pool:Pool){
 ...
 app.get('/api/v1/hoosh/projects',{preHandler:requireAuth},async(req)=>{const a=ar(req);const q=await pool.query('SELECT id,title,description,model_id,mode_id,accent_color,status,created_at,updated_at FROM hoosh_projects WHERE identity_id=$1 AND status=\'active\' ORDER BY updated_at DESC',[a.auth.sub]);return{projects:q.rows};});
