...
async function registerPrivate(app:FastifyInstance){
  ...
  app.patch('/api/v1/banner/listings/:id',{preHandler:requireAuth},async(req,reply)=>{
    ...
    if(b.status!==undefined){if(!['paused'].includes(String(b.status)))return reply.code(400).send({error:'status_invalid'});fields.push({sql:`status=${fields.length+2}`,v:String(b.status)});}
