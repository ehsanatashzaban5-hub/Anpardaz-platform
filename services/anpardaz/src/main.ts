...
app.addHook('onRequest',async(request,reply)=>{if(!iranOnlyPath(request.url))return;const allowed=await requireIranIpInProduction(request,reply);if(allowed===false)return reply;});
