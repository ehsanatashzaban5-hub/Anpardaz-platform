...
let financialSyncTimer:NodeJS.Timeout|undefined;let bankingProviderWorker:BankingProviderWorker|undefined;
if(pool){registerAuthRoutes(app,pool);bankingProviderWorker=new BankingProviderWorker(pool);bankingProviderWorker.start();registerAccountRoutes(app,pool);registerBankingRoutes(app,pool);registerServiceRoutes(app,pool);registerFinnotechBankingRoutes(app,pool);registerFinnotechSayadRoutes(app,pool);registerCardRegistrationRoutes(app,pool);
...
const shutdown=async()=>{if(financialSyncTimer)clearInterval(financialSyncTimer);bankingProviderWorker?.stop();await app.close();await pool?.end()};process.on('SIGTERM',shutdown);process.on('SIGINT',shutdown);await app.listen({host:'0.0.0.0',port});
