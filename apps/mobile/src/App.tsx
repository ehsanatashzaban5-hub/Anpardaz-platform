...
type UserSettingsRecord={theme:"dark"|"light";notificationsEnabled:boolean;keyboardSoundEnabled:boolean;fontScale:number;pinEnabled:boolean;pinUpdatedAt?:string|null;updatedAt?:string|null};
async function getUserSettings():Promise<UserSettingsRecord>{const d=await anpardazRequest("/api/v1/settings");return d.settings as UserSettingsRecord;}
async function updateUserSettings(p:Partial<Pick<UserSettingsRecord,"theme"|"notificationsEnabled"|"keyboardSoundEnabled"|"fontScale">>){return anpardazRequest("/api/v1/settings",{method:"PUT",body:JSON.stringify(p)});}
async function enableUserPin(newPin:string){return anpardazRequest("/api/v1/settings/pin/enable",{method:"POST",body:JSON.stringify({newPin})});}
async function changeUserPin(currentPin:string,newPin:string){return anpardazRequest("/api/v1/settings/pin/change",{method:"POST",body:JSON.stringify({currentPin,newPin})});}
async function disableUserPin(currentPin:string){return anpardazRequest("/api/v1/settings/pin/disable",{method:"POST",body:JSON.stringify({currentPin})});}
