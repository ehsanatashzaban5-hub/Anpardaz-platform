...
function AnTicketsSubPage({onBack}:{onBack:()=>void}){
  ...
  const [tickets,setTickets]=useState<Ticket[]>([]);const[selected,setSelected]=useState<Ticket|null>(null);const[messages,setMessages]=useState<any[]>([]);const[subject,setSubject]=useState("");const[message,setMessage]=useState("");const[loading,setLoading]=useState(true);const[sending,setSending]=useState(false);
  const token=()=>localStorage.getItem("anpardaz:accessToken")??"";
  ...
  if(selected)return <div style={{display:"flex",flexDirection:"column",height:"100%"}}><div style={{padding:"14px 16px",background:"#fff",borderBottom:"1px solid var(--am-border)",display:"flex",gap:10,alignItems:"center"}}><button onClick={()=>{setSelected(null);setMessages([])}} style={{background:"none",border:"none",fontSize:22}}>‹</button><div style={{fontWeight:900}}>{selected.subject}</div></div><div style={{flex:1,overflowY:"auto",padding:16,display:"flex",flexDirection:"column",gap:10}}>{messages.map((m,i)=><div key={i} style={{alignSelf:m.author_type==="user"?"flex-start":"flex-end",maxWidth:"80%",padding:12,borderRadius:14,background:m.author_type==="user"?"var(--am-accent)":"#fff",color:m.author_type==="user"?"#fff":"var(--am-text)",border:"1px solid var(--am-border)"}}>{m.message}</div>)}</div></div>;
