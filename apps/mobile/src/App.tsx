...
function ProfilePage({user,onUpdate,onLogout,lightTheme,setLightTheme}:{user:UserData;onUpdate:(u:UserData)=>void;onLogout:()=>void;lightTheme:boolean;setLightTheme:(v:boolean)=>void}){
    ...
    if(next.length===4){
      setTimeout(async()=>{
        if(pinModal==="enable"){
          ...
          try{
            await userSettingsRequest("/api/v1/user/settings/pin/disable",{method:"POST",body:JSON.stringify({currentPin:next})});
            setPinEnabled(false);onUpdate({...user,pin:""});DB.saveUser({...user,pin:""});setPinModal(null);
          }catch{setPinError("رمز اشتباه است یا غیرفعال‌سازی انجام نشد.");setPinInput("");}
        }
      },100);
    }
    return next;
  ...
  useEffect(()=>{
    ...
    void userSettingsRequest("/api/v1/user/settings").then(({settings})=>{
      ...
      setFontScaleState(settings.fontScale);setPinEnabled(Boolean(settings.pinEnabled));
...
export default function App() {
  ...
  const handleVerified=async(phone:string)=>{DB.setCurrentPhone(phone);const existing=DB.getUser(phone);if(existing){setUser(existing);setTransactions(DB.getTx(phone));const hs=localStorage.getItem(`anp_home_services_${existing.uid}`);if(hs){try{const p=JSON.parse(hs);if(Array.isArray(p))setHomeServices(p);}catch{}}const hp=localStorage.getItem(`anp_home_platforms_${existing.uid}`);if(hp){try{const p=JSON.parse(hp);if(Array.isArray(p))setHomePlatforms(p);}catch{}}setShowCashback(localStorage.getItem(`anp_show_cashback_${existing.uid}`)!=="false");let pinEnabled=Boolean(existing.pin);try{const d=await userSettingsRequest("/api/v1/user/settings");pinEnabled=Boolean(d?.settings?.pinEnabled);if(!pinEnabled&&/^\d{4}$/.test(existing.pin)){await userSettingsRequest("/api/v1/user/settings/pin/enable",{method:"POST",body:JSON.stringify({newPin:existing.pin})}).catch(()=>{});pinEnabled=true;DB.saveUser({...existing,pin:""});setUser({...existing,pin:""});}}catch{}setAppState(pinEnabled?"unlock-pin":"ready")}else{const uid=_genUid();const newUser:UserData={uid,name:"",family:"",nationalId:"",birthDate:"",phone,photo:"",pin:"",tomanBalance:0,usdtBalance:0,cryptoBalances:{},cards:[],registeredAt:new Date().toISOString()};DB.saveUser(newUser);DB.setCurrentPhone(phone);setUser(newUser);setTransactions([]);setHomeServices(DEFAULT_HOME_SERVICES);setPendingTour(true);setAppState("ready")}};
  ...
  if(appState==="unlock-pin"&&user)return <PinUnlock user={user} onVerified={()=>setAppState("ready")} onVerifyPin={async(pin)=>{try{await userSettingsRequest("/api/v1/user/settings/pin/verify",{method:"POST",body:JSON.stringify({pin})});return true}catch{return false}}}/>;
