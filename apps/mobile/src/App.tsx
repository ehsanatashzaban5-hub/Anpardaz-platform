...
function ProfilePage({user,onUpdate,onLogout,lightTheme,setLightTheme:setLightThemePersisted}:{user:UserData;onUpdate:(u:UserData)=>void;onLogout:()=>void;lightTheme:boolean;setLightTheme:(v:boolean)=>void}){
  ...
  const applyServerSettings=async()=>{try{const d=await anpardazGetSettings();const s=d?.settings;if(!s)return;
    void setLightThemePersisted(s.theme==="light");setNotifications(Boolean(s.notifications_enabled));setKeySoundEnabled(Boolean(s.keyboard_sound_enabled));setFontScaleState(Number(s.font_scale)||0);
