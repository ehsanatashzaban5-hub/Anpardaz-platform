...
async function anpardazTransfer(sourceAccountId:number,destinationExternal:string,amount:number,currency:string,description:string){return anpardazRequest("/api/v1/transfers",{method:"POST",body:JSON.stringify({sourceAccountId,destinationExternal,amount:String(amount),currency,description,idempotencyKey:crypto.randomUUID()})});}
...
function TransferScreen({user,onUpdate,transactions,onBack,onDone}:{user:UserData;rate:number;onUpdate:(u:UserData,tx:TxRecord)=>void;transactions:TxRecord[];onBack:()=>void;onDone:()=>void}){
  ...
  const transfer=()=>{
    ...
    anpardazTransfer(0,destClean,amountNum,"IRR",note).then((result)=>{
