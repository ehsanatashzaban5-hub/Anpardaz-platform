...
function TetherSwapScreen({user,rate,onUpdate,onBack}:{user:UserData;rate:number;onUpdate:(u:UserData,tx:TxRecord)=>void;onBack:()=>void}){
  ...
  const submit=async()=>{if(rate<=0){setErr("نرخ لحظه‌ای تتر در دسترس نیست؛ لطفاً چند لحظه بعد دوباره تلاش کنید.");return}if(!num){setErr("مبلغ تبدیل را وارد کنید.");return}if(!localStorage.getItem("anpardaz:accessToken")){setErr("ابتدا وارد حساب شوید.");return}const buying=from==="toman";const quantity=buying?result:num;setProcessing(true);setErr("");try{const order=await sarrafPlaceOrder("USDT","TMN",buying?"buy":"sell","market",quantity,undefined,buying?num:undefined);setReceipt({title:"تبدیل واقعی ثبت شد",amount:buying?faFixed(quantity,4)+" دلار تتر":fa(num)+" دلار تتر",detail:"شناسه سفارش: "+String(order?.order?.id??order?.orderId??"—")+" · موجودی پس از تسویه از حساب آن صراف خوانده می‌شود."});setAmount("");}catch(e){setErr(e instanceof Error?e.message:"ثبت تبدیل انجام نشد.");}finally{setProcessing(false);}};
...
function BillsPaymentScreen({data,user,onUpdate,onBack,onDone}:{data:{billType:string;billName:string;billIcon:string;amount:string;inputVal:string;ownerName:string};user:UserData;onUpdate:(u:UserData,tx:TxRecord)=>void;onBack:()=>void;onDone:()=>void}){
  ...
  const pay=()=>{
    ...
    setErr("");setProcessing(true);try{const result=await anpardazServiceExecute("bill_payment",{billType:data.billName,input:data.inputVal,amount:data.amount,otp,cvv2,expiryMonth:expM,expiryYear:expY});setProcessing(false);resetSensitive();const op=String(result?.operation?.operation_id??result?.operationId??"—");const done=String(result?.operation?.status??"")==="completed";onUpdate(user,{id:op,userId:user.phone,type:"service",fromAsset:"toman",toAsset:"toman",amount:0,fee:0,status:done?"done":"pending",createdAt:new Date().toISOString(),note:`قبض ${data.billName} · ${data.inputVal}`,source:"app"});setReceipt({title:done?"پرداخت قبض با موفقیت انجام شد":"درخواست پرداخت قبض ثبت شد",amount:`${data.amount} ریال`,destination:data.billName,status:done?"success":"pending",detail:"شناسه عملیات: "+op});}catch(e){setProcessing(false);setErr(e instanceof Error?e.message:"پرداخت قبض انجام نشد.");}
  };
  return <>
...
function ViolationsPaymentScreen({data,user,onUpdate,onBack,onDone}:{data:{plate:string;amount:string;ownerName:string};user:UserData;onUpdate:(u:UserData,tx:TxRecord)=>void;onBack:()=>void;onDone:()=>void}){
  ...
  const pay=()=>{
    ...
    if(!cvv2){setErr("CVV2 را وارد کنید.");return}
    if(!expM||!expY){setErr("تاریخ انقضا را وارد کنید.");return}
    setErr("");setProcessing(true);try{const result=await anpardazServiceExecute("vehicle_violations",{plate:data.plate,amount:data.amount,otp,cvv2,expiryMonth:expM,expiryYear:expY});setProcessing(false);resetSensitive();const op=String(result?.operation?.operation_id??result?.operationId??"—");const done=String(result?.operation?.status??"")==="completed";onUpdate(user,{id:op,userId:user.phone,type:"service",fromAsset:"toman",toAsset:"toman",amount:0,fee:0,status:done?"done":"pending",createdAt:new Date().toISOString(),note:`خلافی خودرو · ${data.plate}`,source:"app"});setReceipt({title:done?"پرداخت خلافی با موفقیت انجام شد":"درخواست پرداخت خلافی ثبت شد",amount:`${data.amount} ریال`,destination:`پلاک ${data.plate}`,status:done?"success":"pending",detail:"شناسه عملیات: "+op});}catch(e){setProcessing(false);setErr(e instanceof Error?e.message:"پرداخت خلافی انجام نشد.");}
  };
  return <>
  {processing&&<AnPardazLoadingOverlay text="در حال پردازش پرداخت..."/>}
  {receipt&&<TransactionReceipt data={receipt} onClose={()=>{setReceipt(null);onDone();}}/>}
...
function JudiciaryBillScreen({user,onUpdate,onBack,onDone}:{user:UserData;onUpdate:(u:UserData,tx:TxRecord)=>void;onBack:()=>void;onDone:()=>void}){
  ...
  const pay=()=>{
    ...
    setPayErr("");setProcessing(true);try{const result=await anpardazServiceExecute("judiciary_bill",{billId,amount:inquiryAmount,otp,cvv2,expiryMonth:expM,expiryYear:expY});setProcessing(false);resetSensitive();const op=String(result?.operation?.operation_id??result?.operationId??"—");const done=String(result?.operation?.status??"")==="completed";onUpdate(user,{id:op,userId:user.phone,type:"service",fromAsset:"toman",toAsset:"toman",amount:0,fee:0,status:done?"done":"pending",createdAt:new Date().toISOString(),note:`قبض قضائیه · ${billId}`,source:"app"});setReceipt({title:done?"پرداخت قبض با موفقیت انجام شد":"درخواست پرداخت قبض ثبت شد",amount:`${inquiryAmount} ریال`,destination:"قوه قضائیه",status:done?"success":"pending"});}catch(e){setProcessing(false);setPayErr(e instanceof Error?e.message:"پرداخت قبض انجام نشد.");}
  };
...
function PropertyRegBillScreen({user,onUpdate,onBack,onDone}:{user:UserData;onUpdate:(u:UserData,tx:TxRecord)=>void;onBack:()=>void;onDone:()=>void}){
  ...
  const pay=()=>{
    ...
    if(!cvv2){setPayErr("CVV2 را وارد کنید.");return}
    if(!expM||!expY){setPayErr("تاریخ انقضا را وارد کنید.");return}
    setPayErr("");setProcessing(true);try{const result=await anpardazServiceExecute("property_registration",{billId,amount:inquiryAmount,otp,cvv2,expiryMonth:expM,expiryYear:expY});setProcessing(false);resetSensitive();const op=String(result?.operation?.operation_id??result?.operationId??"—");const done=String(result?.operation?.status??"")==="completed";onUpdate(user,{id:op,userId:user.phone,type:"service",fromAsset:"toman",toAsset:"toman",amount:0,fee:0,status:done?"done":"pending",createdAt:new Date().toISOString(),note:`قبض ثبت اسناد · ${billId}`,source:"app"});setReceipt({title:done?"پرداخت قبض با موفقیت انجام شد":"درخواست پرداخت قبض ثبت شد",amount:`${inquiryAmount} ریال`,destination:"ثبت اسناد و املاک",status:done?"success":"pending"});}catch(e){setProcessing(false);setPayErr(e instanceof Error?e.message:"پرداخت قبض انجام نشد.");}
  };
...
function CharityPaymentScreen({data,user,onUpdate,onBack,onDone}:{data:{orgId:string;orgName:string;amount:string};user:UserData;onUpdate:(u:UserData,tx:TxRecord)=>void;onBack:()=>void;onDone:()=>void}){
  ...
  const pay=()=>{
    ...
    if(!cvv2){setErr("CVV2 را وارد کنید.");return}
    if(!expM||!expY){setErr("تاریخ انقضا را وارد کنید.");return}
    setErr("");setProcessing(true);try{const result=await anpardazServiceExecute("charity",{organization:data.orgId??data.orgName,amount:data.amount,otp,cvv2,expiryMonth:expM,expiryYear:expY});setProcessing(false);resetSensitive();const op=String(result?.operation?.operation_id??result?.operationId??"—");const done=String(result?.operation?.status??"")==="completed";onUpdate(user,{id:op,userId:user.phone,type:"service",fromAsset:"toman",toAsset:"toman",amount:0,fee:0,status:done?"done":"pending",createdAt:new Date().toISOString(),note:`نیکوکاری · ${data.orgName} · ${data.amount}`,source:"app"});setReceipt({title:done?"کمک شما با موفقیت ثبت شد":"درخواست کمک ثبت شد",amount:data.amount,destination:data.orgName,status:done?"success":"pending",detail:"شناسه عملیات: "+op});}catch(e){setProcessing(false);setErr(e instanceof Error?e.message:"پرداخت نیکوکاری انجام نشد.");}
  };
  return <>
  <div className="subscreen" dir="rtl">
    <div className="subscreen-header">
