...
const fmtBank=(c:FinancialCard)=>c.bank_name?`${c.bank_name} · `:"";
 ...
 return <div className="anp-full-page" dir="rtl" style={{background:"var(--bg,#071D2C)"}}>
  ...
  <div className="anp-page-body" style={{paddingBottom:40}}>
   ...
   <div style={{display:"grid",gap:8,marginBottom:14}}>{cards.map(c=><div key={c.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:12,borderRadius:14,background:"rgba(255,255,255,.035)",border:"1px solid rgba(255,255,255,.07)"}}>
    <div><div style={{fontWeight:800}}>{fmtBank(c)}کارت •••• {last4(c.last4)}</div><div style={{fontSize:10,color:"#8298a4",marginTop:4}}>{c.currency==="IRR"?"ریال":c.currency} · {c.status==="active"?"فعال":"غیرفعال"}</div></div>
   ...
   {loading?<div style={{padding:35,textAlign:"center",color:"#9fb0bc"}}>در حال دریافت اطلاعات واقعی…</div>:data?.transactions.length?<>
    ...
    <div style={{fontWeight:800,fontSize:13,margin:"10px 0"}}>نمودار دخل و خرج</div>
    <div style={{height:120,display:"flex",alignItems:"flex-end",gap:3,padding:"8px 2px",borderRadius:14,background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.05)",marginBottom:14}}>
      {(() => {
        const buckets = mode==="year"?12:mode==="month"?31:7;
        const now=new Date(range.e.getTime());
        ...
        const max=Math.max(1,...rows.map(x=>Math.max(x.income,x.expense)));
        return rows.map((x,i)=><div key={i} style={{flex:1,height:"100%",display:"flex",alignItems:"flex-end",gap:1}}><div style={{flex:1,height:`${Math.max(2,Math.round(x.income/max*88))}px`,background:"linear-gradient(180deg,#00D6B0,rgba(0,214,176,.3))",borderRadius:"4px 4px 0 0"}}/><div style={{flex:1,height:`${Math.max(2,Math.round(x.expense/max*88))}px`,background:"linear-gradient(180deg,#f87171,rgba(248,113,113,.3))",borderRadius:"4px 4px 0 0"}}/></div>);
      })()}
    </div>
    ...
    <div style={{display:"grid",gap:6}}>{data.transactions.slice(0,50).map(t=><div key={t.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:10,borderRadius:12,background:"rgba(255,255,255,.025)",border:"1px solid rgba(255,255,255,.05)"}}>
      ...
      <b style={{fontSize:11,color:t.direction==="income"?"#00D6B0":"#f87171"}}>{t.direction==="income"?"+":"-"} {fa(Number(t.amount))} {t.currency==="IRR"?"ریال":t.currency}</b>
    </div>)}</div>
