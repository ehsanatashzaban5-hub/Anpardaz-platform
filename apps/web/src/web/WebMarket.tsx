...
export default function WebMarket({ onNavigate }: Props) {
  ...
  const [showCompare, setShowCompare] = useState(false);\n  const [,setMarketVersion]=useState(0);

  useEffect(()=>{(async()=>{try{const url=MARKET_API+"/api/v1/market/catalog?limit=100"+(search?("&q="+encodeURIComponent(search)):"");const r=await fetch(url,{cache:"no-store"});if(!r.ok)throw new Error("market_catalog_unavailable");const d=await r.json();MARKET_PRODUCTS=(d.products??[]).map((p:any)=>({id:String(p.id),title:p.title??"",titleFa:p.title??"",description:p.description??"",price:Number(p.priceMin??0),originalPrice:Number(p.priceMax??0)>Number(p.priceMin??0)?Number(p.priceMax):undefined,images:(p.media??[]).map((m:any)=>m.url).filter(Boolean).concat((p.offers??[]).map((o:any)=>o.image_url).filter(Boolean)).slice(0,6),category:({ "mobile-digital":"digital","laptop-computer":"computers","home-appliance":"home","supermarket":"grocery","fashion":"fashion","beauty-health":"beauty","audio-video":"audio","automotive":"automotive","sports":"sports","baby-kids":"kids","books-culture":"books","tools-industrial":"tools","travel-camping":"travel","pet":"pets","jewelry-gold":"jewelry" } as Record<string,string>)[p.category_slug]??p.category_slug??"other",subcategory:p.category_name_fa,seller:{id:String(p.offers?.[0]?.store_id??"store"),name:p.offers?.[0]?.store_name??"فروشگاه",nameFa:p.offers?.[0]?.store_name??"فروشگاه",rating:0,reviewCount:0,salesCount:0,isVerified:true,joinedAt:""},rating:0,reviewCount:0,stock:1,sold:0,tags:[],specs:p.specs??{}}));setMarketVersion(v=>v+1);}catch(e){console.error(e)}})();},[search]);
...
}:{cart:string[]}){const[orders,setOrders]=useState<any[]>([]);useEffect(()=>{(async()=>{const token=localStorage.getItem("anpardaz:accessToken")??"";if(!token)return;try{const r=await fetch(MARKET_API+"/api/v1/market/me/orders",{headers:{authorization:"Bearer "+token}});if(r.ok){const d=await r.json();setOrders(d.orders??[])}}catch{}})()},[]);return <div><div style={{fontSize:18,fontWeight:900,marginBottom:20}}>فعالیت خرید من</div><div className="w-card" style={{overflow:"hidden"}}>{orders.length===0?<div style={{padding:30,color:"var(--w-muted)",textAlign:"center"}}>سفارش ثبت‌شده‌ای در آن مارکت وجود ندارد.</div>:orders.map((o:any,i:number)=><div key={o.id} style={{display:"flex",gap:16,padding:16,borderBottom:i<orders.length-1?"1px solid var(--w-border)":"none"}}><div style={{flex:1}}>محصول #{o.product_id}<div style={{fontSize:11,color:"var(--w-muted)"}}>عملیات {o.operation_id??"—"} · {o.status}</div></div><b>{fmtIRT(Number(o.unit_price??0))}</b></div>)}</div>{cart.length>0&&<div className="w-card" style={{marginTop:16,padding:16}}>{cart.map(id=><div key={id}>{MARKET_PRODUCTS.find(p=>p.id===id)?.titleFa??id}</div>)}</div>}</div>}

function AlertsSection() {
  const [alerts,setAlerts]=useState<any[]>([]); const [productId,setProductId]=useState(""); const [price,setPrice]=useState(""); const [busy,setBusy]=useState(false);
  const token=()=>localStorage.getItem("anpardaz:accessToken")??"";
  const load=async()=>{try{const r=await fetch(MARKET_API+"/api/v1/market/me/alerts",{headers:{authorization:"Bearer "+token()}});if(r.ok){const d=await r.json();setAlerts(d.alerts??[])}}catch{}};
  useEffect(()=>{void load()},[]);
  const add=async()=>{const id=Number(productId),target=Number(price);if(!Number.isSafeInteger(id)||id<=0||!Number.isFinite(target)||target<=0)return;setBusy(true);try{const r=await fetch(MARKET_API+"/api/v1/market/me/alerts",{method:"POST",headers:{authorization:"Bearer "+token(),"content-type":"application/json"},body:JSON.stringify({productId:id,targetPrice:target,currency:"IRR"})});if(!r.ok)throw new Error();setProductId("");setPrice("");await load()}catch{}finally{setBusy(false)}};
  const remove=async(id:number)=>{try{await fetch(MARKET_API+"/api/v1/market/me/alerts/"+id,{method:"DELETE",headers:{authorization:"Bearer "+token()}});await load()}catch{}};
  return <div style={{maxWidth:700}}><div style={{fontSize:18,fontWeight:900,marginBottom:20}}>هشدار قیمت</div><div className="w-card" style={{padding:20,marginBottom:20}}><div style={{fontSize:13,fontWeight:700,marginBottom:14}}>هشدار جدید</div><div style={{display:"flex",gap:10}}><input value={productId} onChange={e=>setProductId(e.target.value.replace(/\D/g,""))} placeholder="شناسه محصول" className="w-input"/><input value={price} onChange={e=>setPrice(e.target.value.replace(/\D/g,""))} placeholder="قیمت هدف" inputMode="numeric" className="w-input"/><button onClick={()=>void add()} disabled={busy} className="w-btn w-btn-primary" style={{background:"#d97706"}}>افزودن</button></div><div style={{fontSize:11,color:"var(--w-muted)",marginTop:8}}>شناسه محصول از صفحه محصول آن مارکت قابل مشاهده است.</div></div><div style={{display:"flex",flexDirection:"column",gap:10}}>{alerts.map((a:any)=><div key={a.id} className="w-card" style={{padding:16,display:"flex",alignItems:"center",gap:14}}><div style={{flex:1}}><b>{a.title}</b><div style={{fontSize:12,color:"var(--w-muted)",marginTop:4}}>قیمت هدف: {fmtIRT(Number(a.target_price))}</div></div><button onClick={()=>void remove(Number(a.product_id))} className="w-btn w-btn-ghost">حذف</button></div>)}</div></div>;
}
