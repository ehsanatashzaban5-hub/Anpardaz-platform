...
function BannerChatImage({id}:{id:string}){const [src,setSrc]=useState("");useEffect(()=>{let u="";let dead=false;(async()=>{try{const t=localStorage.getItem("anpardaz:accessToken")??localStorage.getItem("accessToken")??"";const r=await fetch(API+"/api/v1/banner/messages/"+encodeURIComponent(id)+"/media",{headers:{authorization:"Bearer "+t},cache:"no-store"});if(!r.ok)return;u=URL.createObjectURL(await r.blob());if(!dead)setSrc(u)}catch{}})();return()=>{dead=true;if(u)URL.revokeObjectURL(u)}},[id]);return src?<img src={src} alt="تصویر پیام" style={{maxWidth:260,maxHeight:320,borderRadius:10}}/>:<span style={{fontSize:11,color:"var(--w-muted)"}}>در حال دریافت تصویر…</span>}
...
function Messages({conversations,conversation,setConversation,onRefresh}:{conversations:Conversation[];conversation:any;setConversation:(x:any)=>void;onRefresh:()=>void}){
 const [text,setText]=useState("");const [imageBusy,setImageBusy]=useState(false);
 const open=async(id:string)=>{try{setConversation(await api("/api/v1/banner/conversations/"+id))}catch{}};
 const send=async()=>{if(!conversation?.conversation?.id||!text.trim())return;await api("/api/v1/banner/conversations/"+conversation.conversation.id+"/messages",{method:"POST",body:JSON.stringify({message:text.trim(),type:"text"})});setText("");setConversation(await api("/api/v1/banner/conversations/"+conversation.conversation.id));onRefresh()};
 const sendImage=async(file:File)=>{if(!conversation?.conversation?.id||!["image/jpeg","image/png","image/webp"].includes(file.type)||file.size>8*1024*1024)return;setImageBusy(true);try{const data=await new Promise<string>((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(String(fr.result).split(",")[1]??"");fr.onerror=()=>reject(fr.error);fr.readAsDataURL(file)});await api("/api/v1/banner/conversations/"+conversation.conversation.id+"/messages",{method:"POST",body:JSON.stringify({type:"image",mediaMime:file.type,mediaBase64:data,message:""})});setConversation(await api("/api/v1/banner/conversations/"+conversation.conversation.id));onRefresh()}catch{}finally{setImageBusy(false)}};
 if(conversation)return <div><button className="w-btn w-btn-ghost" onClick={()=>setConversation(null)}>بازگشت</button><div className="w-card" style={{padding:15,marginTop:10}}><h3>{conversation.conversation.message||"گفت‌وگو"}</h3><div style={{display:"grid",gap:8}}>{(conversation.messages??[]).map((m:any)=><div key={m.message_id} style={{padding:9,borderRadius:9,background:"var(--w-card2)"}}>{m.media_mime?<BannerChatImage id={String(m.message_id)}/>:m.body}</div>)}</div><textarea className="w-input" value={text} onChange={e=>setText(e.target.value)} placeholder="پیام" style={{marginTop:10}}/><div style={{display:"flex",gap:8,marginTop:7}}><input id="banner-chat-image" type="file" accept="image/jpeg,image/png,image/webp" style={{display:"none"}} onChange={e=>{const f=e.target.files?.[0];if(f)void sendImage(f);e.currentTarget.value=""}}/><button className="w-btn w-btn-ghost" onClick={()=>document.getElementById("banner-chat-image")?.click()} disabled={imageBusy}>ارسال تصویر</button><button className="w-btn w-btn-primary" onClick={()=>void send()} disabled={!text.trim()}>ارسال</button></div></div></div>;
