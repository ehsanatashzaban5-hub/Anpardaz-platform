    ...
    <div style={{background:"var(--w-surface)",borderBottom:"1px solid var(--w-border)"}}>
      <div style={{maxWidth:1480,margin:"0 auto",padding:"10px 20px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",gap:8,flex:1,minWidth:260}}>
          ...
          <select value={category} onChange={e=>setCategory(e.target.value)} className="w-input" style={{maxWidth:190}}><option value="">همه دسته‌ها</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select>
        </div>
...
function Detail({listing,favorite,onBack,onFavorite,isLoggedIn,onAuthRequired}:{listing:Listing;favorite:boolean;onBack:()=>void;onFavorite:()=>void;isLoggedIn:boolean;onAuthRequired:()=>void}){
 ...
 return <div><button className="w-btn w-btn-ghost" onClick={onBack}>بازگشت</button><div style={{display:"grid",gridTemplateColumns:"minmax(0,1.4fr) minmax(300px,.8fr)",gap:18,marginTop:12}}>
   ...
   <div className="w-card" style={{padding:18,height:"fit-content"}}><div style={{fontWeight:900}}>فروشنده</div><div style={{marginTop:10}}>{listing.seller_display_name??"کاربر آن بنر"}</div><button className="w-btn w-btn-ghost" onClick={onFavorite} style={{marginTop:12}}><WI n={favorite?"heartFill":"heart"} s={15}/> {favorite?"حذف از علاقه‌مندی":"افزودن به علاقه‌مندی"}</button><textarea className="w-input" rows={5} value={message} onChange={e=>setMessage(e.target.value)} placeholder="پیام به فروشنده" style={{marginTop:14}}/><button className="w-btn w-btn-primary" onClick={()=>void send()} style={{marginTop:8,width:"100%",justifyContent:"center"}}>ارسال پیام</button>{sent&&<div style={{color:"#059669",fontSize:12,marginTop:8}}>پیام در سرویس آن بنر ثبت شد.</div>}</div>
 </div></div>;
}
...
function Post({categories,onDone}:{categories:any[];onDone:()=>void}){
 const [categoryId,setCategoryId]=useState("");const [title,setTitle]=useState("");const [description,setDescription]=useState("");const [price,setPrice]=useState("");const [condition,setCondition]=useState("used");const [city,setCity]=useState("");const [files,setFiles]=useState<File[]>([]);const [busy,setBusy]=useState(false);const [err,setErr]=useState("");
 const submit=async()=>{if(!categoryId||!title.trim()||!description.trim()||!city.trim())return;setBusy(true);setErr("");try{const x=await api("/api/v1/banner/listings",{method:"POST",body:JSON.stringify({categoryId:Number(categoryId),title:title.trim(),description:description.trim(),price:price?Number(price):null,condition,city:city.trim(),currency:"IRR"})});for(const f of files.slice(0,8)){const data=await new Promise<string>((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(String(r.result).split(",")[1]??"");r.onerror=()=>reject(r.error);r.readAsDataURL(f)});await api("/api/v1/banner/listings/"+x.listing.id+"/media",{method:"POST",body:JSON.stringify({mimeType:f.type,filename:f.name,dataBase64:data})})}onDone()}catch(e){setErr(e instanceof Error?e.message:"ثبت آگهی ناموفق بود")}finally{setBusy(false)}};
 return <div className="w-card" style={{padding:22,maxWidth:760}}><h2>ثبت آگهی</h2><div style={{display:"grid",gap:10}}><select className="w-input" value={categoryId} onChange={e=>setCategoryId(e.target.value)}><option value="">انتخاب دسته</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><input className="w-input" value={title} onChange={e=>setTitle(e.target.value)} placeholder="عنوان آگهی"/><textarea className="w-input" value={description} onChange={e=>setDescription(e.target.value)} rows={6} placeholder="توضیحات"/><input className="w-input" value={price} onChange={e=>setPrice(e.target.value)} placeholder="قیمت به ریال" inputMode="numeric"/><select className="w-input" value={condition} onChange={e=>setCondition(e.target.value)}><option value="new">نو</option><option value="like-new">در حد نو</option><option value="good">خوب</option><option value="used">کارکرده</option><option value="for-parts">قطعات</option></select><input className="w-input" value={city} onChange={e=>setCity(e.target.value)} placeholder="شهر"/><input type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={e=>setFiles(Array.from(e.target.files??[]).slice(0,8))}/>{err&&<div style={{color:"#dc2626",fontSize:12}}>{err}</div>}<button className="w-btn w-btn-primary" onClick={()=>void submit()} disabled={busy}>{busy?"در حال ثبت...":"ثبت آگهی برای بررسی"}</button></div></div>;
}
