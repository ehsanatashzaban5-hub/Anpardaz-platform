...
function Messages({conversations,conversation,setConversation,onRefresh}:{conversations:Conversation[];conversation:any;setConversation:(x:any)=>void;onRefresh:()=>void}){
 ...
 if(conversation)return <div><button className="w-btn w-btn-ghost" onClick={()=>setConversation(null)}>بازگشت</button><div className="w-card" style={{padding:15,marginTop:10}}><h3>{conversation.conversation.message||"گفت‌وگو"}</h3><div style={{display:"grid",gap:8}}>{(conversation.messages??[]).map((m:any)=><div key={m.message_id} style={{padding:9,borderRadius:9,background:"var(--w-card2)"}}>{m.media_mime?<img src={API+"/api/v1/banner/messages/"+encodeURIComponent(m.message_id)+"/media"} alt="تصویر پیام" style={{maxWidth:260,maxHeight:320,borderRadius:10}}/>:m.body}</div>)}</div><textarea className="w-input" value={text} onChange={e=>setText(e.target.value)} placeholder="پیام" style={{marginTop:10}}/><div style={{display:"flex",gap:8,marginTop:7}}><input id="banner-chat-image" type="file" accept="image/jpeg,image/png,image/webp" style={{display:"none"}} onChange={e=>{const f=e.target.files?.[0];if(f)void sendImage(f);e.currentTarget.value=""}}/><button className="w-btn w-btn-ghost" onClick={()=>document.getElementById("banner-chat-image")?.click()} disabled={imageBusy}>ارسال تصویر</button><button className="w-btn w-btn-primary" onClick={()=>void send()} disabled={!text.trim()}>ارسال</button></div></div></div>;
